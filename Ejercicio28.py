...
# Ejercicio 28 - Curvas de contagio, muerte y recuperación de los 10 departamentos con mas casos de contagiados acumulados

datos_ag = datosCovid.groupby(['Nombre departamento'])
dep_cont = datos_ag.size().sort_values(ascending=False).head(10)

fig = plt.figure(figsize=(12, 5))

fa = datosCovid[datosCovid['Estado'] == 'Fallecido'].groupby('Nombre departamento')
muerte = fa.size().sort_values(ascending=False).head(10)

r = datosCovid[datosCovid['Recuperado'] == 'Recuperado'].groupby('Nombre departamento')
...
plt.title("Grafico de contagio, muerte y recuperación departamento")
plt.show()
